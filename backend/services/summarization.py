...
def analyze_transcript(transcript: str) -> MeetingAnalysisSchema:
    ...
    if provider == "mock":
        return MeetingAnalysisSchema(**MOCK_ANALYSIS)
    ...
    if provider == "openai":
        ...
        if not settings.GEMINI_API_KEY:
            raise ValueError("GEMINI_API_KEY is missing in environment/configuration (.env). Set GEMINI_API_KEY=... or LLM_PROVIDER=mock.")
        try:
            ...
            raise RuntimeError(f"Gemini LLM analysis failed: {str(e)}")
